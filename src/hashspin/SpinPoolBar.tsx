  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-1.5">
        <span className="font-display tracking-widest" style={{ color: '#6872a0', fontSize: '0.6rem' }}>POOL</span>
        <span className="font-mono font-bold" style={{ color: '#e8eeff', fontSize: '0.8rem' }}>
          {fmtBtc(round.totalPool)} BTC
          {btcPrice ? ` · ${fmtUsd(round.totalPool, btcPrice)}` : ''}
        </span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 1, height: 20, alignItems: 'flex-end' }}>
        {round.slotPools.map((pool, i) => {
          const height = pool > 0 ? Math.max(3, (pool / maxPool) * 20) : 1;
