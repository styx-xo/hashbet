...
export function SpinTimer({ round, lastBlockTimestamp }: Props) {
  ...
  useEffect(() => {
    if (!round || round.phase !== 'BETTING') {
      if (round?.phase === 'SPINNING' && frozenAt.current === null) frozenAt.current = elapsed;
      return;
    }
      ...
      <AnimatePresence mode="wait">
        {round.phase === 'BETTING' && (
          <motion.div key="betting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            {/* Compact: status + timer + bar all tight */}
        ...
        {round.phase === 'SPINNING' && (
          <motion.div key="spinning" className="text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        ...
        {round.phase === 'SETTLED' && round.winnerSlot !== undefined && (
          <motion.div key="settled" className="flex items-center justify-between" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <span className="font-display font-black" style={{ fontSize: '1.2rem', color: slotColor(round.winnerSlot), textShadow: `0 0 16px ${slotColor(round.winnerSlot)}88` }}>
              {SLOT_LABELS[round.winnerSlot]} WINS
            </span>
            <span className="font-mono text-xs text-text-dim animate-pulse">Next round...</span>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
